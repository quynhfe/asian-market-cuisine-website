...
export default function VisitUsSection({ locale, dictionary }: VisitUsSectionProps) {
  const visitUs = dictionary.home.visitUs;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-32 items-center">
          <div className="order-2 lg:order-1">
            <div className="h-[300px] md:h-[500px] w-full rounded-lg overflow-hidden contrast-125 editorial-shadow">
              <Image alt="Da Nang Map" className="w-full h-full object-cover" src={IMAGES.visitUsHome} />
            </div>
          </div>

          <div className="order-1 lg:order-2">
            <span className="font-label uppercase tracking-[0.2em] text-[10px] text-on-surface-variant mb-4 block">{visitUs.overline}</span>
            <h2 className="font-headline italic text-4xl md:text-6xl mb-8 md:mb-12">{visitUs.title}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-8 mb-12">
              {[
                { label: visitUs.addressLabel, value: visitUs.addressValue },
                { label: visitUs.hoursLabel, value: visitUs.hoursValue },
              ].map(({ label, value }) => (
                <div key={label}>
            ...
            <div className="flex flex-col sm:flex-row gap-4 md:gap-6">
              <a href={localizeHref("/find-us", locale)} className="w-full sm:w-auto bg-on-surface text-background px-6 py-4 rounded-full font-label uppercase tracking-[0.15em] text-xs flex items-center justify-center gap-3 hover:bg-primary transition-all cursor-pointer focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-on-surface">
